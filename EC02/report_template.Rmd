...
```{r loaddata}
dados <- read.csv("C:/Users/João Paulo/Documents/planejamento_e_analise_experimentos/EC02/DadosAcoesGrupoG.csv", header = FALSE)
...
de forma a compreendermos qual a melhor forma de realizarmos os experimentos
...
estatísticos devem ser realizados para poder afirmar tal suposta conclusão.
...
gerar o poder do teste esperado para nosso experimentos.
...
FALAR SE ESTÁ BOM OU NÃO E O QUE PODEMOS FAZER E SE OS TESTES CONTINUARÃO CONFIÁVEIS.
...
Podemos concluir que os resíduos de fato seguem suficientemente uma distribuição normal padrão. Por fim, precisamos de checar a igualdade de variâncias entre cada um dos fatores, ou seja, sua homoscedasticidade.
...
Para verificar se o número de amostras utilizado é adequado para realizar o teste de comparação, foi conduzido um teste t considerando um nível de significância ($\alpha$) de 0,05, um poder estatístico de 0,8 e uma diferença mínima detectável ($\delta$) correspondente a um desvio padrão dos resíduos do modelo ANOVA.

```{r power t test}
sd<-sqrt(var(my.model$residuals))
